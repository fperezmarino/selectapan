odoo.define('school.session.inherit', require => {
    // "use strict";
    //
    ...
    //
    //         // District codes
    //         const current_district_id = session.user_districts.current_district.id;
    //         if (!state.districtCodeIds) {
    //             state.districtCodeIds = utils.get_cookie('districtCodeIds') !== null ? utils.get_cookie('districtCodeIds') : String(current_district_id);
    //         }
    //
    //         let stateDistrictCodeIDS = _.map(state.districtCodeIds.split(','), cid => parseInt(cid));
    //         const userDistrictCodeIDS = _.map(session.user_districts.allowed_districts, dc => dc.id);
    //         // Check that the user has access to all the companies
    //         if (!_.isEmpty(_.difference(stateDistrictCodeIDS, userDistrictCodeIDS))) {
    //             state.districtCodeIds = String(current_district_id);
    //             stateDistrictCodeIDS = [current_district_id]
    //         }
    //
    //
    //         // School codes
    //         const current_school_id = session.user_schools.current_school.id;
    //         if (!state.schoolCodeIds) {
    //             state.schoolCodeIds = utils.get_cookie('schoolCodeIds') !== null ? utils.get_cookie('schoolCodeIds') : String(current_school_id);
    //         }
    //
    //         let stateSchoolCodeIDS = _.map(state.schoolCodeIds.split(','), cid => parseInt(cid));
    //         const userSchoolCodeIDS = _.map(session.user_schools.allowed_schools, dc => dc.id);
    //         // Check that the user has access to all the companies
    //         if (!_.isEmpty(_.difference(stateSchoolCodeIDS, userSchoolCodeIDS))) {
    //             state.schoolCodeIds = String(current_school_id);
    //             stateSchoolCodeIDS = [current_school_id]
    //         }
    //         session.user_context.allowed_district_ids = stateDistrictCodeIDS;
    //         session.user_context.allowed_school_ids = stateSchoolCodeIDS;
    //
    //         $.bbq.pushState({
    ...
    //
    // Session.include({
    //     setSchoolCodes: function (main_district_id, district_ids,
    //                               main_school_id, school_ids) {
    //         var hash = $.bbq.getState()
    //         hash.districtCodeIds = district_ids.sort(function(a, b) {
    //             if (a === main_district_id) {
    //                 return -1;
    //             } else if (b === main_district_id) {
    //                 return 1;
    //             } else {
    //                 return a - b;
    //             }
    //         }).join(',');
    //
    //         hash.schoolCodeIds = school_ids.sort(function(a, b) {
    //             if (a === main_school_id) {
    //                 return -1;
    //             } else if (b === main_school_id) {
    //                 return 1;
    //             } else {
    //                 return a - b;
    //             }
    //         }).join(',');
    //
    //         utils.set_cookie('districtCodeIds', hash.districtCodeIds || String(main_district_id));
    //         utils.set_cookie('schoolCodeIds', hash.schoolCodeIds || String(main_school_id));
    //
    //         $.bbq.pushState({
