...
class medical_patient_ambulatory_care(models.Model):
    ...
    _description = "Medical Patient Ambulatory Care"
    ...
    @api.constrains('session_end')
    def _check_session_end(self):
        for rec in self:
          if rec.session_end <= rec.session_start :
            raise UserError(_('Session End Date Not Less Than Session Start Date.' ))
